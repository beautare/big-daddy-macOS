...
- 孩子可以在菜单里用与"安全退出"相同的一次性验证码**关闭**（本机覆盖）。覆盖会挡住这份配置，直到家长在 Dashboard 把连续性**关掉再打开**（客户端看到 `false → true` 边沿时清掉覆盖）。
...
> 打开后，守护程序在崩溃或被强制退出后会自动再打开。孩子看得到菜单栏图标；关闭程序仍需要你生成的一次性验证码。把 BigDaddy 从电脑上删除后，它不会自己装回来。
