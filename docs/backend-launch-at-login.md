...
- 家长在 Dashboard 生成的临时验证码，孩子既可以用它正常退出客户端，**也可以用它关闭开机自启**、**关闭崩溃后自动恢复**（三者是相互独立的敏感操作，共用同一次验证）。
- 正因为一码三用，全线文案统一叫"临时验证码"（英文 `temporary verification code`），不要再叫"退出码/退出验证码"——那会让家长误以为这个码只能用于退出。这条已落地：Dashboard 的 `getExitToken`/`exitTokenTitle`/`exitTokenHint` 与客户端三处弹窗均已改用该措辞（i18n key 名保留 `exitToken*` 未改，只改展示文案）。
